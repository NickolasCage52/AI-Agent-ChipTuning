...
async def cmd_start(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        ...
        "• <i>OEM 04465-33480</i> или артикул\n\n"
        "Чем больше деталей (марка, модель, год, передние/задние) — тем точнее подбор.\n"
        "Я уточню недостающее и подберу варианты по цене: Эконом / Оптимум / OEM.",
        parse_mode="HTML",
    )
...
async def cmd_help(message: Message) -> None:
    await message.answer(
        ...
        "<b>Что помогает подобрать точнее:</b>\n"
        "• Марка и модель авто\n"
        "• Год выпуска\n"
        "• Передние или задние (для колодок, стоек)\n"
